		...
		stage('Initial Test SSH') {
			steps {
				withCredentials([
					...
					//sh 'aws s3 cp s3://fresh-test-bucket/SK_Tama_Black_JPEG.jpg /home/ec2-user/'
		...
		stage ('Pull file from bucket') {
			steps {
				withAWS(credentials: 'aws_creds', region: 'eu-west-1') {
					sh 'echo "Random test text" > random.txt'
					s3Upload acl: 'Private', bucket: 'fresh-test-bucket', file: 'random.txt'
				}
			}
		}
